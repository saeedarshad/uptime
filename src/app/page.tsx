...
function Wordmark({ dark = false }: { dark?: boolean }) {
  return (
    <div className="flex items-center gap-2.5">
      ...
      <span
        className={`text-xl font-bold tracking-tight ${dark ? "text-white" : "text-graphite"}`}
      >
        UptimeHQ
      </span>
    </div>
...
function LandingHeader({ loggedIn }: { loggedIn: boolean }) {
  return (
    <header className="sticky top-0 z-40 border-b border-graphite/[0.06] bg-paper/80 backdrop-blur-md">
      <div className="mx-auto flex h-16 max-w-6xl items-center justify-between px-5 sm:px-8">
        ...
        <nav className="hidden items-center gap-8 md:flex">
          <a
            href="#features"
            className="text-sm font-medium text-graphite/70 transition-colors hover:text-graphite"
          >
            Features
          </a>
          <a
    ...
    <section className="relative overflow-hidden">
      ...
      <div className="mx-auto max-w-6xl px-5 pb-16 pt-16 sm:px-8 sm:pb-24 sm:pt-24">
        <div className="mx-auto max-w-3xl text-center">
          <span className="inline-flex items-center gap-2 rounded-full border border-graphite/10 bg-white px-3.5 py-1.5 text-xs font-semibold text-graphite/70 shadow-sm">
            <span className="h-1.5 w-1.5 rounded-full bg-ok" />
            Flat price · Unlimited users · No per-seat fees
          </span>
          <h1 className="mt-6 text-4xl font-bold leading-[1.08] tracking-tight text-graphite sm:text-6xl">
          ...
          <p className="mx-auto mt-6 max-w-xl text-lg leading-relaxed text-graphite/60">
          ...
          <p className="mt-4 text-sm text-graphite/40">
...
function DashboardPreview() {
  ...
  return (
    <div className="mx-auto max-w-4xl">
      <div className="overflow-hidden rounded-2xl border border-graphite/10 bg-white shadow-elevated">
        <div className="flex items-center gap-1.5 border-b border-graphite/[0.06] bg-graphite/[0.02] px-4 py-3">
          ...
          <span className="ml-3 rounded-md bg-graphite/[0.05] px-3 py-1 text-[11px] font-medium text-graphite/40">
        ...
        <div className="grid gap-4 p-5 sm:grid-cols-3">
          ...
          {[
            { label: "Downtime", value: "41 hrs", tone: "text-graphite" },
            { label: "Spend (90d)", value: "$3,345", tone: "text-safety" },
            { label: "PM compliance", value: "50%", tone: "text-ok" },
          ].map((s) => (
            <div key={s.label} className="rounded-xl border border-graphite/[0.08] p-4">
          ...
          <div className="rounded-xl border border-graphite/[0.08] p-4 sm:col-span-3">
            <div className="mb-3 text-[11px] font-medium uppercase tracking-wide text-graphite/50">
              Monthly spend
            </div>
...
function LogoStrip() {
  ...
  return (
    <section className="border-y border-graphite/[0.06] bg-white/50">
      <div className="mx-auto max-w-6xl px-5 py-8 sm:px-8">
        <p className="text-center text-xs font-semibold uppercase tracking-widest text-graphite/40">
        ...
        <div className="mt-5 flex flex-wrap items-center justify-center gap-x-10 gap-y-3">
          {types.map((t) => (
            <span
              key={t}
              className="text-base font-bold tracking-tight text-graphite/30"
            >
              {t}
            </span>
          ))}
  ...
  return (
    <section id="features" className="mx-auto max-w-6xl px-5 py-20 sm:px-8 sm:py-28">
      <div className="mx-auto max-w-2xl text-center">
        ...
        <h2 className="mt-3 text-3xl font-bold tracking-tight text-graphite sm:text-4xl">
      ...
      <div className="mt-14 grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
        {FEATURES.map((f) => (
          ...
          >
            ...
            <h3 className="mt-4 text-base font-bold text-graphite">{f.title}</h3>
...
function HowItWorks() {
  return (
    <section id="how" className="border-y border-graphite/[0.06] bg-white/50">
        ...
        <div className="overflow-hidden rounded-2xl border border-graphite/10 bg-white shadow-elevated">
          ...
          <div className="p-8">
            <ul className="space-y-3">
              {includes.map((i) => (
                <li key={i} className="flex items-center gap-3 text-sm text-graphite/80">
...
function Footer() {
  return (
    <footer className="border-t border-graphite/[0.06]">
      <div className="mx-auto flex max-w-6xl flex-col items-center justify-between gap-4 px-5 py-10 sm:flex-row sm:px-8">
        ...
        <div className="flex items-center gap-6 text-sm font-medium text-graphite/60">
          <Link href="/login" className="hover:text-graphite">
            Sign in
          </Link>
          <Link href="/register" className="hover:text-graphite">
            Start free
          </Link>
        </div>
      </div>
